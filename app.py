...
def generate():
    ...
    system = """You are a GEMS Education PE lesson designer using the TLAG framework. Return ONLY a valid JSON object with no markdown fences.
...
Schema:
{
  ...
  "exitTicket": {"q1":"","wordBank":["","",""],"q2":""}
    ...
    user = f"""Generate a TLAG PE lesson.
...
LO: students will be able to {d['lo']}{swim_line}"""
    ...
    try:
        ...
        import json
        lesson = json.loads(msg.content[0].text)
